...
function AuthPage() {
  ...
  async function handleLinkedIn() {
    const redirect = safePath(sessionStorage.getItem("operon:redirect") ?? destination);
    sessionStorage.setItem("operon:redirect", redirect);
    // Redirect to our server-side LinkedIn OAuth flow
    window.location.href = `/api/auth/linkedin?redirect=${encodeURIComponent(redirect)}`;
  }
